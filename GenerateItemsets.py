...
#set location of training data file

cur_dir = "~/Kaggle/CrimeStatsSF/data/"
cur_dir = os.path.expanduser(cur_dir)
training_file = cur_dir + "train.csv"
...
print "Printing itemsets, category counts and itemset counts to file..."

#output counts to file
fileOut = cur_dir + "CategoryCountsPerItemsetLength.dat"
with open(fileOut, 'w+') as fO:
    for number_items in range(0,max_size_itemsets + 1):
        fO.write(str(total_count_itemsets[number_items]))
        for category in range(0,n_categories+1):
            fO.write(','+str(counts_categories[number_items][category]))
        fO.write('\n')

#output counts to file
fileOut = cur_dir + "ItemsetLengthCountsPerCategory.dat"
with open(fileOut, 'w+') as fO:
    for category in range(0,n_categories + 1):
        fO.write(str(total_count_categories[category]))
        for number_items in range(0, max_size_itemsets + 1):
            fO.write(','+str(counts_itemsets[category][number_items]))
        fO.write('\n')


#output itemsets to file
index = 0
file_out = cur_dir + "RawItemsets.dat"
...
for i_length in range(1,max_size_itemsets+1):
    ...
    #save plot to file
    figName = cur_dir + 'distr_itemset_length_' + str(i_length) + '.png'
    pl.savefig(figName)
    pl.close()
...
for i_category in range(1,n_categories+1):
    ...
    #construct barchart
    ax.bar(labels, incidents, width=1)
    #pl.title('Incidents in Itemsets of Length '+ str(i_length), bbox={'facecolor':'0.8', 'pad':5})
    ...
    #save plot to file
    figName = cur_dir + 'distr_category_' + str(i_category) + '.png'
    pl.savefig(figName)
    pl.close()